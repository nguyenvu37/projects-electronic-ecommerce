export const formatNumberUSD = (num) => {
  ...
  if (indexCharDot !== -1) arrCharBefore = arrNum.slice(0, indexCharDot).reverse();

  if (indexCharDot === -1) arrCharBefore = [...arrNum].reverse();
  ...
  if (indexCharDot !== -1) return  arrCharResult.join('') + arrCharAfter;
  if (indexCharDot === -1) return arrCharResult.join('') + '.00';
};
